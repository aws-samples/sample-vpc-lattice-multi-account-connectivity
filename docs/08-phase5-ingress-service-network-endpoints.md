...
> **How this phase is delivered.** Unlike Phases 1 through 4, which ship as deployment-tested CDK stacks and CloudFormation templates, this phase is a **reference design**: it is presented as AWS CLI steps plus an automation blueprint (the EventBridge, Step Functions, and Route 53 flow below), not as a ready-to-deploy stack in this repository. The SN-E itself is a single `create-vpc-endpoint` call, and the DNS automation closely follows the productized [Guidance for Amazon VPC Lattice Automated DNS Configuration on AWS](https://aws.amazon.com/solutions/guidance/amazon-vpc-lattice-automated-dns-configuration-on-aws/), which you can deploy directly. Treat the steps here as the design to adapt, and reach for that Guidance solution when you want maintained IaC for the cross-account automation.
