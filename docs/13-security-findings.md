...
| T9 | Spoofing | Rogue Interface VPC endpoint planted in the Endpoint VPC is mapped to a service domain (CloudFormation path: the VPCE DNS lookup enumerates all interface endpoints in the VPC) | High | M9 | In Progress |
...
| T14 | Information Disclosure | Compromised VPCE DNS lookup Lambda role (`ec2:DescribeVpcEndpoints` on `*`, **CloudFormation path only**) enumerates endpoint inventory for recon | Low-Medium | M11 | Resolved |
...
- **M11**, Least-privilege IAM on the one custom-resource Lambda that remains (the **CloudFormation-path** VPCE DNS lookup): scoped actions, a documented `ec2:DescribeVpcEndpoints` wildcard justification, a short timeout, a dead-letter queue, and reserved concurrency limit the recon value of a compromised role. The CDK path resolves endpoint DNS natively from the endpoint's `DnsEntries`, and the workload association (both paths) resolves the VPC ID via a native `AWS::SSM::Parameter::Value` with the service network ID passed as a parameter, so **no lookup Lambda or lookup role exists on those paths**. *Addresses T14; eliminates T15 by design.*
...
- **Justified `Resource: '*'` uses**, each on an action that AWS does not allow to be resource-scoped:
  ...
  4. **CloudFormation path only:** `ec2:DescribeVpcEndpoints` on the VPCE DNS lookup Lambda, `Describe*` has no resource-level support (relates to **T14**). The CDK path has no equivalent, because it reads endpoint DNS natively from `DnsEntries`.
- **No `ListServiceNetworks`, and no lookup-Lambda role anywhere.** The workload association passes the service network ID as a parameter and resolves the VPC ID via a native `AWS::SSM::Parameter::Value`, so there is no `vpc-lattice:ListServiceNetworks` grant and no lookup role in either path.
