...
> **A note on conventions.** As elsewhere in this guide, examples use the `us-east-2` Region and placeholder identifiers: organization ID `o-EXAMPLE12345`, account `111111111111`, and OU ARNs of the form `arn:aws:organizations::111111111111:ou/o-EXAMPLE12345/ou-EXAMPLE`. The reference IaC names the three networks `sn-dev-shared` / `sn-test-shared` / `sn-prod-shared` in both the CDK and CloudFormation paths; this section refers to them generically as the dev, test, and prod Service Networks.
...
// snTest ('sn-test-shared') and snProd ('sn-prod-shared') follow the same shape
```
...
The representative policy below is the dev network's auth policy from the CloudFormation `DevServiceNetworkAuthPolicy`. It permits the dev environment's OU path; the `/*` suffix matches that OU and everything nested beneath it. If an environment spans more than one OU, list each OU path here.
      ...
      "Condition": {
        ...
        "ForAnyValue:StringLike": {
          "aws:PrincipalOrgPaths": [
            "o-EXAMPLE12345/r-EXAMPLE/ou-EXAMPLE-root/ou-EXAMPLE-dev0000/*"
          ]
        }
      }
    }
...
The test and prod networks carry the same policy shape, differing only in the OU path they permit (`TestOuPath` and `ProdOuPath`). The practical effect: a principal in the dev OU is allowed by the dev network and denied by the prod network, because its org path matches no prod OU path.
...
Both IaC paths share each network to a single OU ARN per environment:
...
| Principals per env | **One OU ARN** (`devOuArn` / `testOuArn` / `prodOuArn`) | **One OU ARN** (`DevOuArn` / `TestOuArn` / `ProdOuArn`) |
...
// RamShareTest -> props.testOuArn, RamShareProd -> props.prodOuArn
```

The CloudFormation template shares each environment's network to its environment OU ARN:
...
If an environment spans more than one OU, add every OU ARN for that environment to the share's `Principals` and list each OU path in the auth policy's `aws:PrincipalOrgPaths`, keeping the two in sync.
...
The relevant stack is `VpcLatticeCoreStack` in `cdk/lib/vpc-lattice-core-stack.ts`, instantiated in `cdk/bin/app.ts` with context props `orgId`, `devOuArn`, `testOuArn`, and `prodOuArn`. Supply those values as CDK context, either inline with `-c` or in `cdk.json`, then deploy:
...
npx cdk deploy VpcLatticeCoreStack \
  ...
  -c testOuArn=arn:aws:organizations::111111111111:ou/o-EXAMPLE12345/ou-EXAMPLE-test \
  -c prodOuArn=arn:aws:organizations::111111111111:ou/o-EXAMPLE12345/ou-EXAMPLE-prod
```

The stack exports `serviceNetworkIds` (dev/test/prod) as both a TypeScript property and CloudFormation outputs (`ServiceNetworkDevId`, `ServiceNetworkTestId`, `ServiceNetworkProdId`). The endpoints stack and the egress stack consume these IDs and depend on this stack, so deploy it first. Remember that the CDK path does not attach the OU-path auth policies, add them as described in Step 1.
...
A representative deployment passes the OrgId with the per-environment OU ARN and OU path parameters:
...
aws cloudformation deploy \
  ...
  --parameter-overrides \
    ...
    DevOuPath=o-EXAMPLE12345/r-EXAMPLE/ou-EXAMPLE-root/ou-EXAMPLE-dev0000 \
    TestOuArn=arn:aws:organizations::111111111111:ou/o-EXAMPLE12345/ou-EXAMPLE-test000 \
    TestOuPath=o-EXAMPLE12345/r-EXAMPLE/ou-EXAMPLE-root/ou-EXAMPLE-test000 \
    ProdOuArn=arn:aws:organizations::111111111111:ou/o-EXAMPLE12345/ou-EXAMPLE-prod0000 \
    ProdOuPath=o-EXAMPLE12345/r-EXAMPLE/ou-EXAMPLE-root/ou-EXAMPLE-prod0000
```
...
- **Three Service Networks** (dev, test, prod), each with `AuthType: AWS_IAM` and an `Environment` tag.
