...
| Security | Defense in depth across two boundaries, Service Network IAM auth policies (OU-path scoped) and OU-scoped RAM shares with external principals disabled, plus centralized FQDN-allowlist egress filtering and least-privilege IAM |
...
| Implement least privilege | Scoped Lambda roles grant only `ssm:GetParameter` on `/apg-lattice/*` (or your equivalent SSM prefix) and the unavoidable `vpc-lattice:ListServiceNetworks` (an action that does not support resource-level permissions, documented with a `cdk-nag` `AwsSolutions-IAM5` suppression). Security groups are explicit, no default VPC security group, no `0.0.0.0/0`. |
