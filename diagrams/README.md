...
| **Egress data flow** | A DNS step (step 0) with numbered steps 1-6 with a sidebar: workload (HTTP_PROXY) → Lattice / Service Network → egress Resource Gateway ENI → internal NLB :3128 → Squid on Fargate (FQDN allowlist decision: allow vs. deny) → NAT Gateway → internet. The Egress VPC container uses the purpose color teal (`#0E7490`). |
| **Ingress data flow** | A DNS step (step 0) with numbered steps 1-4 with a sidebar: an external, on-premises, or cross-Region consumer resolves a custom domain (a CNAME/Alias to the SN-E generated DNS name, automated via EventBridge + Step Functions + Route 53) → connects to the SN-E IP over Direct Connect / VPN / peering → SN-E carries the request privately over PrivateLink into the Service Network (the IAM auth policy still applies) → Resource Gateway → shared resource (endpoint, proxy, or app target). The Ingress VPC container uses the purpose color purple (`#9333A8`). |
...
- **Two complementary color systems:**
  ...
  - **Environment colors** for the Service Networks and the per-environment Workload VPCs: **dev = green (#1B660F)**, **test = amber (#CC8800)**, **prod = maroon (#8B0000)**.
